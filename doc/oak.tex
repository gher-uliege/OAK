...
\mathbf{S}^f
\left[
\mathbf I-(\mathbf I+{\mathbf{U\Lambda U^T})}^{-1}(\mathbf{U\Lambda U^T}+\mathbf I - \mathbf I)
\right]{\mathbf{S}^f}^T \label{PaSfSft2} \\
&=&
\mathbf{S}^f
\mathbf{U}
{\left(\mathbf +\mathbf{\Lambda}\right)}^{-1}\mathbf{U}^T {\mathbf{S}^f}^T \label{PaSfSft}
\end{eqnarray}

using $\mathbf{UU}^T = \mathbf I$ from \eqref{PaSfSft2} to \eqref{PaSfSft}.
