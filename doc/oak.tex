...
RRSQRT, ESSE, EnKF) we decompose $\mathbf P^f$ in terms of its square root 
matrices $\mathbf S^f$:
...
In practise, the following eigenvalue decomposition is made,
...
diagonal. $\mathbf U$ and $\Lambda$ are both of the size $r \times
r$. The Kalman gain $\mathbf K$ and $\mathbf S^a$ can be computed
by:
