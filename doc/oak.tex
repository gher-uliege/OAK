...
\begin{equation}
\mathbf{\overline{x^{\{a,f\}}}}=\frac{1}{N}\sum_{k=1}^Nx^{\{a,f\}(k)} \label{ensmean}
\end{equation}
be respectively the analysis (a) and forecast (f)  ensemble mean.
We construct the matrix $\mathbf S^f$ by:
\begin{equation}
\mathbf{S}^f=\frac{\mathbf{x}^f(k)-\mathbf{\overline{x}^f}}{\sqrt{N-1}} \label{Saf}
\end{equation}
...
\begin{eqnarray}
\mathbf{K}&=&{\mathbf{S}^f\mathbf{S}^f}^T\mathbf{H}^T\left[(\mathbf{HS}^f)(\mathbf{HS}^f)^T+\mathbf{R}\right]^{-1}\label{beforeSherman}\\
&=&\mathbf{S}^f\left[\mathbf{I}+(\mathbf{H}\mathbf{S}^f)^T\mathbf{R}^{-1}(\mathbf{HS}^f)\right]^{-1}(\mathbf{H}\mathbf{S}^f)^T\mathbf{R}^{-1}\label{afterSherman}
\end{eqnarray}
...
\begin{equation}
{\left(\mathbf{A}^{-1}+\mathbf{B}^T\mathbf{C}^{-1}\mathbf{B}\right)}^{-1}\mathbf{B}^T\mathbf{C}^{-1}=\mathbf{AB}^T{\left(\mathbf{C}+\mathbf{BAB}^T\right)}^{-1}
\end{equation}

with $\mathbf A=1$, $\mathbf B=\mathbf{HS}^f$, $\mathbf{C}=\mathbf{R}$.
That is, instead of performing the inverse in space of matrix $\mathbf{A}$ the inverse is done in space of matrix $\mathbf C$.\newline
Furthermore, in practise, the following eigenvalue decomposition is made :
\begin{equation}
...
diagonal.
%The Kalman gain $\mathbf K$ and $\mathbf S^a$ can therefore be computed
%by:
%\begin{equation}
 %\mathbf K = \mathbf S^f \mathbf U (1+ \mathbf \Lambda)^{-1} \mathbf U^T (\mathbf H \mathbf S^f)^T \mathbf R^{-1}\label{KU}
%\end{equation}
 $\mathbf U$ and $\Lambda$ are both of the size $r \times r$.\newline  
Using \eqref{Udec} in \eqref{afterSherman} and injecting \eqref{afterSherman} and \eqref{PfSf} in \eqref{PaPf} leads to

\begin{eqnarray}
\mathbf{P}^a&=&\mathbf{S}^f\left[1-(1+{(\mathbf{U\Lambda U^T})}^{-1}\mathbf{U\Lambda U^T}\right]{\mathbf{S}^f}^T\\
&=&\mathbf{S}^f\left[1-(1+{\mathbf{U\Lambda U^T})}^{-1}(\mathbf{U\Lambda U^T}+1-1)\right]{\mathbf{S}^f}^T \label{PaSfSft2} \\
&=&\mathbf{S}^f\mathbf{U}{\left(1+\mathbf{\Lambda}\right)}^{-1}\mathbf{U}^T {\mathbf{S}^f}^T \label{PaSfSft}
\end{eqnarray}

using $1=\mathbf{UU^T}$ from \eqref{PaSfSft2} to \eqref{PaSfSft}.
So we found a decomposition of $\mathbf{P}^a=\mathbf{S}^a{\mathbf{S}^a}^T$ with $\mathbf{S}^a=\mathbf{S}^f\mathbf{U}(1+\Lambda)^{-1/2}$. But  in order to ensure \eqref{ensmean}, given \eqref{Saf}, the mean of $\mathbf S^a$ has to be zero, so is the mean of $\mathbf S^f$. So we will transform $\mathbf S^a$ so that the identity \eqref{PaSfSft} is preserved. One way to do this is
