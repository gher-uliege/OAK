...
! Ef = randn(n,Nens)
 ...
 ! set observed value
 call getInitValue(initfname,'Obs001.value',names)
 call usave(trim(path)//names(i),yo,DefaultValex)
 deallocate(names)

 ! set RMSE
 call getInitValue(initfname,'Obs001.rmse',names)
 call usave(trim(path)//names(i),sqrt(diag(R)),DefaultValex)
 deallocate(names)
